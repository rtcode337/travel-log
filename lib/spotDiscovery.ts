...
/**
 * 同じものを指す書き方の組。**全文検索は表記が違うと当たらない**ので、
 * 検索語をこの組で言い換えて何度か引く(実測: ある繁華街のbboxを片仮名で引くと、
 * 同じ語を平仮名で名乗る店が1軒も出ず、平仮名で引き直して初めて出た)。
 *
 * カテゴリでの絞り込み(`lib/overtureNearby.ts` / `lib/osmNearby.ts`)とは別物。
 * あちらは「名前に出ない語」を拾うための表で、こちらは**名前の書き方のゆれ**を埋める。
 */
const SPELLING_GROUPS: string[][] = [
  ["ラーメン", "らーめん", "ラー麺", "らー麺", "中華そば"],
  ["うどん", "饂飩"],
  ["そば", "蕎麦", "そば処"],
  ["寿司", "すし", "鮨", "スシ"],
  ["カフェ", "喫茶", "珈琲", "コーヒー"],
...
/**
 * 全文検索に投げる書き方の一覧(先頭は渡された語そのもの)。
 *
 * **3文字未満の語は落とす** —— chiezoの全文検索は3文字未満をタイトルの前方一致に
 * 落とすので(`そば`・`鮨`など)、そのまま投げると別物の結果が混ざる。
 * 渡された語自体は短くてもそのまま残す(呼び出し側の意図なので、こちらで削らない)。
 */
export function searchSpellings(word: string): string[] {
  const q = word.trim();
  if (!q) return [];
  const group = SPELLING_GROUPS.find((words) => words.some((w) => q.includes(w)));
  if (!group) return [q];
  const out = [q];
  for (const w of group) {
    if (w.length >= 3 && !q.includes(w) && !out.includes(w)) out.push(w);
  }
  return out;
}
