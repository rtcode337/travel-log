...
`PREFECTURES`(47都道府県ハードコード)を「地域の全集合」として使ってよいのは`'jp'`スコープの文脈だけ、という点に注意。
...
**Wikipediaのための設定はもう持たない。** かつては種別ごとに`wikipedia_enabled`(リンクを出すか)・`wikipedia_lang`(言語版)・`wikipedia_title_source`(スポット名で探すかシリーズ名で探すか)の3つを持ち、記事の概要をその場で出すモーダル(`SpotInfoModal`)がそれらを読んでいた。**記事へ行く導線が説明文のリンクだけになったので、3つとも設定として意味を失った**(言語版も記事名もデータ側の`[<言語版>.wikipedia:<記事名>]`が持つ)。設定もモーダルも、それを呼んでいた地図の追加確認の「Wikipediaで見る」も廃し、追加確認にはスポット詳細と同じGoogleの導線を置いた(→[スポットについて生成AIに聞く](#スポットについて生成aiに聞くlibaskaits))。**`spot_type_settings`に残った古い行は読まれないだけ**なので、消すための移行は要らない。
...
**この並びは`components/GoogleSpotLinks.tsx`の1か所**で、スポット詳細と
**地図の訪問予定リスト作成中の追加確認**が共用する —— どちらも「この場所へ行くか」を
決める場面なので、片方にだけ導線があると探しに戻ることになる。
**アイコンだけの行にする**(文字のリンクを混ぜると狭い画面で折り返し、
アイコンの列が2行に割れて読みにくくなる)。経路の出発地に使う現在地は
この部品の中で引く(`useRouteOrigin`。権限が既に許可されているときだけ)。
