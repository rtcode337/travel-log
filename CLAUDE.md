...
| 段 | 口 | かかる時間 | 得意なもの |
|---|---|---|---|
| **地図データ**(必ず走る) | `GET /api/spots/nearby` | **2〜9秒**(半径による。下記) | 正確な座標。AIの枠を使わない |
| **AIで補う**(チェックしたときだけ) | `POST /api/spots/discover` | 17秒〜2分 | 地図に無い新しい店、一言の説明 |

**かつては「地図データ / AIでweb検索」の二択だった**が、AIは単品で使うものではなく
**地図データに足りないぶんを埋めるために使う**、というのが実際の使われ方だった。
探索の画面のチェックボックス「AIに足りないぶんを補わせる」を入れると、
地図データを出したあとで続けてAIにも聞き、同じ一覧に足す。
...
1回に3分近くかかって使い物にならなかった。**まず地図データで雑に集め、足りなければ
AIで探し足す**(パネルの「もう一度探す」。同じ一覧に混ざり、行に出どころの印が付く)。
...
**件数の選択(`DISCOVERY_LIMIT_OPTIONS`)はAIで探すときだけ出す。** 返させる件数がそのまま
待ち時間になるのはAIの側の事情で、地図データには当てはまらない。

**限界は測ってある**: どちらの辞典も開いたばかりの店は載らず、営業時間も持たない
(OSMで入っているのは新宿で23%・郡山で1%)。**だからAIの段を残してある。**

#### 2段目: AIでweb検索(`app/api/spots/discover/route.ts`)
...
**一覧の行を薄くするのは「登録済み」と「地図データで実在を確かめられなかった」の2つだけ**。
...
AIなら「AIがwebから収集(日付)、参照: URL」。**どの辞典かは候補ごとに持つ**
