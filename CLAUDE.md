...
**一覧は「使った値」で育つ。** POST `/api/spots`に`?register_series=1`・`?register_categories=1`を付けると、追加したスポットの**シリーズ・カテゴリのうち一覧にまだ無いものを末尾へ足す**(`registerUsedValues`+`mergeSeriesStyles`/`mergeCategories`)。既定値を廃したぶんの受け皿で、地図から手で足したスポットのシリーズ・カテゴリがここを通る。
...
左の柱の中も触る頻度の順に並べる —— GitHubからの取り込み → スポットの管理 →
このスポット種別の管理 → 別のスポット種別の管理 → ログイン後に自動で開く種別。
取り込み以外は畳んでおく。右の柱の**ユーザー管理も既定で畳む**(ユーザーを足す・
変えるのはたまにで、開いたままだと縦に長い一覧が右の柱を占める)。
...
「自分の記録」で、タブが無かったときと同じものが最初に出る。**探し方の既定は◯◯
(都道府県など)で、タブでも柱の中の切り替えでも◯◯を先に並べる**)。**切り替わる幅は段組みと同じ**
(スポット一覧は`sm`、管理画面は`lg`)——タブの`sm:hidden`/`lg:hidden`と、
グリッドの`sm:grid-cols-2`/`lg:grid-cols-[1fr_360px]`は必ずそろえること。

**スポット一覧の「探す」は、タブのときだけ探し方ごとに分ける**(「探す: ◯◯」
「探す: シリーズ」。◯◯は対象地域の呼び名=`regionFieldLabel`)。右の柱には探し方の切り替えが
...
公開スポットの中身が怪しい(位置がずれている・説明が別物・そもそも場所ではない)、
あるいは**地図にあるはずのスポットが無い**と気づいたときに、**その場で印だけ
付けておく**ための仕組み。直すのは受け取った側(収集を回す側・travel-log-dataの
CSV)なので、アプリ側は「気づいた」を貯めて渡すところまでを受け持つ。
**表・APIの名前は`spot_flags`、画面の言葉は「修正・追加の依頼」**
—— 「フラグ」だけでは何のための印か画面から読めないため。
かつては「間違い報告」と呼んでいたが、足りないスポットも同じ一覧で渡すようになり、
「間違い」では追加の依頼を言い表せなくなった。

- **修正の依頼はスポット詳細(`SpotDetailModal`)の地図の右上「⚠ 修正を依頼」**
  (公開スポット・spot_admin/adminのみ。重ね表示=readOnlyでは出さない)。
  押すと理由の入力欄が開き、**空のまま依頼できる** —— 理由を書かせると、
  「あとで書く」つもりで依頼そのものをしないまま流れてしまう。
  依頼済みの間はボタンが「⚠ 依頼を取り消す」になり、地図の下に理由が出る
- **追加の依頼は地図の右クリック(長押し)の「ここにスポット追加を依頼」**
  (spot_admin/adminのみ。「ここにスポットを追加」の下)。**スポットは作らない**
  —— 承認待ちに積むのではなく、依頼の一覧に座標と理由だけを残す。
  `spot_id`は空で、`lat`/`lng`/`spot_type_id`を持つ(どちらか一方は必須。check制約)。
  **自分が出した依頼は、片付くまで地図に印で出る**(`GET /api/spot-flags?type=&mine=1`。
  未依頼は破線・対応中は実線)。追加の依頼は緑の「+」(タップで理由)、修正の依頼は
  スポットの座標(ピンの先端)を囲む黄色の輪 —— 追加の依頼はスポットが作られず、
  修正の依頼もピンの見た目は変わらないので、印が無いとどこに頼んだか・もう頼んだかが
  地図から分からなくなる。**輪はタップを素通しにする**(ピンをそのまま押せるように。
  理由はスポット詳細に出る)。ピンの大きさはランクで変わるので、頭に札を載せるより
  先端を囲むほうがずれない
- **依頼はスポットに何の影響も与えない。** 公開状態も地図の見え方も変わらない
  (承認待ち・却下は別の流れ。依頼は「直す候補のメモ」でしかない)
- **修正の依頼は1スポットに1つ**(`spot_flags.spot_id`がユニーク。nullどうしは
  別物なので追加の依頼は何件でも入る)。同じスポットに依頼し直すと理由が上書きされる。
  スポットが消えれば修正の依頼も、種別が消えれば追加の依頼も消える(`on delete cascade`)
- **一覧は`/[type]/admin`の「⚠ 修正・追加の依頼」**(spot_admin/admin)。
  [修正]/[追加]の印・名前(地図へのリンク。追加の依頼は座標をGoogleマップで開く)・
  理由・依頼した人・日時を、未依頼を先にして依頼された順に並べる。**行ごとの「削除」を持つ** ——
  追加の依頼は指す先のスポットが無いので、ほかに消す場所が無い
  (`DELETE /api/spot-flags/[id]`はスポットのidでも依頼のidでも消せる)
- **未依頼と対応中を分けて持つ**(`spot_flags.forwarded_at`。空なら未依頼、入っていれば
  対応中)。テキストを受け取る側へ渡したら「渡したので対応中にする」で印を付ける
  (行ごとの「対応中にする」/「未依頼に戻す」もある。`PATCH /api/spot-flags`に
  `{ ids, forwarded }`)。**一括はテキストに並べたidで指す** —— 表示してから押すまでに
  増えた依頼まで渡したことにしないため。修正の依頼を出し直すと未依頼に戻る
  (理由が変われば渡した中身とは別の依頼になる)。スポット詳細の表示にも「(対応中)」が付く
- **一覧の下の「渡すテキスト」は未依頼のものだけを、1行に名前・座標・理由で並べる**
  (対応中のものまで入れると同じ依頼を二重に渡してしまう)。**開閉させずに常に出す** ——
  節ごと管理画面の折り畳みの中にあるので、ここでさらに畳むと渡すまでに開く操作が2段になる
  (`- [修正] 名前 (緯度,経度): 理由` / `- [追加] (緯度,経度): 理由`)。
  そのままAIに貼って直し方を相談するほか、**tazunaの「travel-logからの依頼」に貼ると、
  座標を含む区画の情報収集にこのテキストを補足として載せて回せる**。
  **座標を入れるのはそのため** —— 追加の依頼は名前を持たないので場所は座標でしか言えず、
  受け取る側は座標から「どの区画の話か」を引く(名前だけでは同名の店を取り違える)。
  キーは入れない(判断の材料にならない)。`CopyTextButton`で写せる。
  **理由の無いものは理由を書かない** —— 「(理由なし)」と書くと、
  AIがその文字列を理由として読んでしまう
- **一括の削除は「未依頼を一括で削除」と「対応中を一括で削除」に分ける**
  (一覧の下の同じ行の右端)。片付けは「渡す→対応中にする→直ったら対応中を一括で削除」の
  流れを想定していて、要らなくなった未依頼だけを消すこともある。
  **いま一覧に出ている依頼のidで指す**(`DELETE /api/spot-flags?type=`に`{ ids }`。
  種別に属するものだけを消す)—— 状態で消すと、表示してから押すまでに増えた依頼まで
  見ないまま消してしまう。画面の言葉は「取り消す」ではなく「削除」 —— 行ごと消えて、
  戻す手段は無いため
