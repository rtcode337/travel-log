...
- ピンのクリックで**2つのピンレイヤーを`queryRenderedFeatures`で見て**、複数見つかったとき(=タップ位置に複数のピンが重なっている)は先頭を開かず**「この地点のスポット」一覧**(`stackSpotIds` state)を出してどれを開くか選ばせる。選ぶと通常どおり`handleSpotSelect`に流れるため、作成モード中の追加確認にもそのまま繋がる。**一覧は横幅を`max-w-sm`に留める一方、高さは`max-h-[85vh]`まで使う**(重なりが数十件になる種別があるため、件数が多いときに画面の高さいっぱいまで伸ばし、はみ出す分だけ一覧側をスクロールさせる)
