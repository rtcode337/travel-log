...
| 段 | 口 | かかる時間 | 得意なもの |
|---|---|---|---|
| **地図データ**(既定) | `GET /api/spots/nearby` | **240ms以下**(実測) | 正確な座標。AIの枠を使わない |
| **AIでweb検索** | `POST /api/spots/discover` | 17秒〜2分 | 地図に無い新しい店、一言の説明 |
...
同じものを並べられる。違いは`source`(`osm`/`ai`)だけで、画面はそれを印にする。
...
chiezoのOSM辞典(`osm_japan`)を引くだけ。**探し方は2つ投げて混ぜる**(`lib/osmNearby.ts`):

- **全文検索**(`/search`)は「ラーメン」のように**名前に出る語**に強い
- **種別での絞り込み**(`/filter?feature=`)は「ランチ」のように**名前に出ない語**に要る。
  片方だけだと、どちらかの語で何も出ない。`feature`はOSMの主タグ1つしか受け付けない
  (`cuisine=ramen`で絞ろうとすると0件。実測)ので、語→主タグの表を持つ
- 全文検索の応答には座標が載らないので、当たった題名で`doc`を引き直す。**並列に投げる**

**座標は地図データそのもの**なので`location_verified`は常にtrue。ジャンルは`cuisine`→
主タグの順で日本語に直す。**説明文は持たない**(OSMに無いので`summary`はnull)。
**日本の種別だけ**(chiezoのOSM辞典が日本の抽出)。

**限界は測ってある**: 新宿1kmに飲食店884件あるのに、AIが挙げた有名店4件のうち
OSMで引けたのは1件だけだった(タカノフルーツパーラーのような老舗も載っていない)。
営業時間が入っているのは新宿で23%・郡山で1%。**だからAIの段を残してある。**
...
**AIの答えは信用せず確かめてから出す。** 位置はchiezoのOSM辞典を候補ぶん並列に引き、
当たれば「位置確認済み」、当たらなければAIの座標のまま「未確認」(新店はOSMに無いのが
普通なので珍しくない)。重複は名前一致か50m以内で「登録済み」。**口コミ本文・点数・
...
候補は番号つきの印(AIは紫、位置未確認は破線の縁、登録済みは灰色、チェックを外したものは薄く)で、
...
(地図データなら「OpenStreetMapから取得(日付)」、AIなら「AIがwebから収集(日付)、参照: URL」)
—— 手で書いた説明と見分けるため。ジャンルはカテゴリに入れる。状態・シリーズは全行共通。
公開で追加した行は`spotCache.applySpotChange`で次のダウンロードを待たずに地図へ載せる。

**OSMのライセンス(ODbL)について。** この機能が引くのは**自分のインスタンスで使うため**で、
配布ではない(地図表示には`© OpenStreetMap contributors`を出している)。ただし
`origin='manual'`は還元用エクスポートの抽出条件なので、**OSM由来の名前・座標を
travel-log-data(MIT側)へ持ち込まないよう、還元するときは出どころを見て外すこと**
(「外部データソース…注意」の「ヒントに留める」と同じ線)。
