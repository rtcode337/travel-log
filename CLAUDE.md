...
(`DiscoveryExchange`・`AiExchangeDialog`。パネルの「AIとのやり取りを見る」)。
...
**パネルの出どころバッジと同じ色**にしてある —— 片方だけ変えると一覧と地図の対応が切れる。
...
**狭い画面では右の帯ではなく下から敷く帯にする**(`top-[35%]`で高さ65%、`sm:`から
従来の`right-0 w-2/5 max-w-sm`)。右の帯のままだとスマホの幅では150px前後しか残らず、
**行の中身が1文字ずつ縦に折り返されて読めない**。高さを65%取っているのは、
見出しと下の操作で上下を挟むため —— 半分だと一覧が1行ぶんも残らない
(あわせて狭い画面では見出しの説明文を伏せ、下の操作の余白を詰める)。
...
チェック・×(候補から外す)・行ごとのランク・「もう一度探す」(**同じ一覧に足す**。
...
—— 手で書いた説明と見分けるため。ジャンルはカテゴリに入れる。状態・シリーズは全行共通。
