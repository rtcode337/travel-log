...
- **一覧は`/[type]/admin`の「⚠ 修正・追加の依頼」**(spot_admin/admin)。
  ...
  理由・依頼した人・日時を、未依頼を先にして依頼された順に並べる。**行ごとの「取り消す」を持つ** ——
  追加の依頼は指す先のスポットが無いので、ほかに取り消す場所が無い
...
- **「テキストで表示」は未依頼のものだけを、1行に名前・座標・理由で並べる**
  (対応中のものまで入れると同じ依頼を二重に渡してしまう)
...
- **「依頼を一括で取り消す」は種別ぶんをまとめて消す**(`DELETE /api/spot-flags?type=`)。
  片付けは「渡す→直す→まとめて取り消す」の流れを想定している
