...
あわせて`wikipedia_lang`キー(既定`'ja'`、`resolveWikipediaLang`)でWikipediaを引くときの言語版サブドメインを種別ごとに切り替えられる(記事の概要を出す`SpotInfoModal`と、スポット詳細から記事へ飛ぶリンクが使う)。

**Wikipediaの記事へのリンクは、説明文に`[ja.wikipedia:汽車道]`と書いてある** —— 表示側(`LinkedText`)がWikipediaのアイコン+記事名のリンクに描く(→[説明文の中のURLをリンクにする](#説明文の中のurlをリンクにする-linkedtext))。**表示側で本文を解釈して出どころを組み直さない。** かつては`(出典: ja.wikipedia「◯◯」 https://…)`という日本語の文を切り分けてリンクに直していたが、書き方が少し変わるだけで壊れ、データを見ても何がリンクになるのか分からなかった。**行を分けるのもデータ側の改行**で、travel-log-dataの説明文は本文の後ろに改行で`掲載元: [ja.wikipedia:神奈川県の観光地]`を並べている。

**かつてスポット詳細にあったWikipediaのアイコンボタン(押すと記事の概要を`SpotInfoModal`で出す)は廃した** —— 説明文の中に記事へのリンクが出るようになり、同じ記事へ行く導線が2つになったため(`SpotInfoModal`自体は地図の「訪問予定に追加」の確認から今も使う)。
...
**よく使う期間はカレンダーを開かずに選べる。** 訪問日セクションとカレンダーに**「今日」「表示しない」**を並べ、見出し行に**「過去1年」**のトグルを置いてある(`MapView`の`visitDateOptions`)。**1年前の日付は`Date`に年だけ引かせる**ので、2月29日は3月1日に送られる(閏年でない年に2月29日は無い)——絞り込みの範囲としては1日の差なので、月末を特別扱いはしない。
...
スポット詳細のGoogleの並びに**Geminiのボタン**を置いてある。押すと**質問文を入れた状態**で
Google検索のAIモード(`https://www.google.com/search?udm=50&q=`。`buildGeminiAskUrl`)が
新しいタブで開く。`gemini.google.com`はURLで質問文を渡せない(2026-08時点でも非対応。
第三者のChrome拡張で補う方法しか無い)ため、**同じGeminiが答えるAIモードを使っている**。

**聞ける相手はGeminiだけにしてある。** かつては押してから相手(Claude・ChatGPT・Perplexity・
Grok)を選ぶ「AIに聞く」ボタン(`components/AskAiButton.tsx`)を別に並べていたが、2026-09-21に
外した —— `?q=`の渡し方も、開いた時点でそのまま送信されるかどうかもサービスごとに違ううえ
予告なく変わり(ChatGPTは「外部リンクからは自動送信しない」と告知しながら実際は送信される)、
確かめ直し続ける手間に見合わなかった。相手を増やすなら、当時の実装が
`git log -- components/AskAiButton.tsx`に残っている。

**Googleの並びは軽いものから重いものへ**(地図 → 経路 → 画像検索 → Gemini)。
押せばすぐ答えが出るものを先に、読むのに時間がかかるものを最後に置く。

**画像検索も同じ並びにある**(`udm=2`が画像タブ。AIモードの`udm=50`と同じ渡し方)。
文章より写真のほうが早い場面があるため —— 見た目が分かれば「行くかどうか」も
...
見出しの横の「?」を押すと説明を吹き出しで出す共通部品。**長い説明書きを畳んで画面を短く保つ**ためのもので、出し方は2つ。**中身は同じで、違うのは吹き出しをどこへ描くかだけ**。

| 指定 | 出る位置 | 使いどころ |
|---|---|---|
| (既定) | `fixed`で「?」の真下(画面の下半分にある「?」は真上) | ほぼ全部 |
| `sheet` | `fixed`で画面の下端 | 真下・真上のどちらにも収まらないときや、地図の上のように吹き出しが操作の邪魔になるとき |

**`absolute`で出す既定は廃した。** `overflow`で切り取る箱(スクロールするモーダル・一覧、角丸のための`overflow-hidden`)の中では欠け、横スクロールまで生えていた —— 呼ぶ側が`anchored`を付けるかどうかで直るのは、**付け忘れが起きるだけ**で得が無い。いまは`sheet`以外すべて`fixed`で「?」の位置に合わせて描く(旧`anchored`が既定になった形)。

**横は「?」の左端に合わせ、右へはみ出すときだけ右端に合わせる。** 画面の右寄りにある「?」を左端合わせのままにすると吹き出しが画面の外まで伸びる。最後に画面の内側へ寄せて両端に余白(`ANCHORED_MARGIN`)を残す。位置は開いた時点の`getBoundingClientRect`で決め、スクロール(capture付き)とリサイズで追従させる。**位置が決まるまでは`invisible`にする** —— 一瞬だけ画面の左上に出てから飛ぶため。

### 説明文の中のURLをリンクにする(`LinkedText`)

スポット・経路・訪問予定リストの説明文に出てくる`http(s)://…`と、**`[ja.wikipedia:記事名]`の書き方**を`<a>`にして描く共通部品。使うのは`SpotDetailModal`(スポット詳細)・`MapView`(訪問予定への追加確認・経路詳細・区間の説明)・`VisitPlanListDetailModal`(リストの説明)。

- **Wikipediaの記事はURLではなくアイコン+記事名で出す**(`[ja.wikipedia:汽車道]`→ Wikipediaのロゴ+「汽車道」)。URLは`https://<言語版>.wikipedia.org/wiki/<記事名をパーセントエンコード>`でこちらが組むので、**データ側は言語版と記事名だけ書けばよい**(丸括弧付きの記事名「ハーバー (菓子)」もそのまま書ける)。記事名は`]`まで、改行は含めない
- **リンクになるものはデータ側に書く。** 表示側で日本語の文(「出典: …」)を解釈してリンクに組み直さない —— 書き方が少し変わるだけで壊れ、データを見ても何がリンクになるのか分からない
- アイコンは`components/WikipediaIcon.tsx`の1か所に置く(地図の追加確認のボタンと同じ絵にするため)
...
- **`dangerouslySetInnerHTML`は使わない。** 説明文は管理画面から誰でも書けるので、HTMLとして解釈させるとそこがXSSの口になる。`href`に入るのも正規表現が拾った`http(s)://…`と、言語版と記事名から組んだWikipediaのURLだけで、`javascript:`はそもそも一致しない
...
**絞り込みモーダルの節は、見出しの大きさを全部そろえる**(`text-sm font-medium`)。「絞り込み」も訪問日・訪問予定リスト・表示と並んだ節の1つでしかなく、大きくすると他の節まで絞り込みの下にあるように読める(実際、絞り込みのリセットが効くのはシリーズ・カテゴリ・訪問状況だけ)。**節の長い説明書きは見出し横の`HelpTip`に畳む** —— モーダルは縦に伸ばせないので、説明を出しっぱなしにすると下の節がスクロールの先へ逃げる。
...
重ね表示のスポットをタップすると読み取り専用のスポット詳細(`SpotDetailModal`の`readOnly`prop。スポット自体の編集・削除・承認/却下や非表示の切り替えを出さない)、ルートをタップすると本体と共用のルート詳細モーダルが開く。読み取り専用でも**訪問記録**(`allowVisitRecording`)と**訪問予定リストへの追加**(`allowPlanList`)だけは行える — どちらも対象は**今開いている地図の種別**(URLの`[type]`)で、重ねられた側の種別のリストは扱わない(リストの経由スポット`visit_plan_list_items`は種別非依存のため、別種別のスポットもそのまま現在の種別の旅程に混ぜられる。地図の作成モードで別種別のピンから追加できるのと同じ)。**別種別の地図へ移す導線はスポット詳細に置いていない**(かつては「「◯◯」の地図で開く」を出していたが、アイコンの並ぶ行に文字のリンクが混ざって狭い画面で折り返すため外した。種別の切り替えは地図の左下の種別チップから行う)。種別チップ以外から`?spot=<id>&from=<元の種別キー>`で遷移してきたときは、遷移先の地図は`from`がある間、左下の種別チップの上に「← 「◯◯」の地図に戻る」リンクを出す(`/[from]/map`への遷移のみで、元いた表示位置は種別ごとの`lastViews`がそのまま復元する。`?spot=`処理後のURL整理でも`from`は消さずに残し、地図タブ以外へ遷移するとリンクは消える)。

### 2カラムの画面は、狭いときタブにする(スポット一覧・管理画面)

`/[type]/admin`は**左がスポットの管理、右がユーザー管理と訪問記録のエクスポート**
(`lg:grid-cols-[1fr_360px]`。右はadmin専用なので、adminでなければ1本だけ)。
**日常的に触るのはスポットの側**なので、広い画面では読み始める左に置く。

**狭い画面ではタブで切り替える**(`AdminView`の`adminTab`。既定は「スポット」)。
縦に積むと、上の柱を全部スクロールしないと下の柱に届かない —— ユーザー管理は縦に長く、
その下にスポットの管理が埋まっていた。**タブはadminのときだけ出す**(切り替える相手が
いなければ要らない)。

**`/[type]/spots`も同じ形**(`SpotsView`の`spotsTab`。左=自分の記録(訪問予定・訪問予定リスト・
最近の訪問場所・口コミ・非公開・非表示)、右=「◯◯から探す / シリーズから探す」。既定は
「自分の記録」で、タブが無かったときと同じものが最初に出る)。**切り替わる幅は段組みと同じ**
(スポット一覧は`sm`、管理画面は`lg`)——タブの`sm:hidden`/`lg:hidden`と、
グリッドの`sm:grid-cols-2`/`lg:grid-cols-[1fr_360px]`は必ずそろえること。

**スポット一覧の「探す」は、タブのときだけ探し方ごとに分ける**(「探す: シリーズ」
「探す: ◯◯」。◯◯は対象地域の呼び名=`regionFieldLabel`)。右の柱には探し方の切り替えが
もう1段あり、タブで柱を選んでからもう一度選ぶことになるため —— タブが出ている幅では
柱の中の切り替えを出さない(`hidden sm:flex`)。タブの値は`spotsTab`と`browseMode`から
組み立てる(状態を1つに混ぜると、広い画面のときに探し方が読めなくなる)。

**切り替えは`hidden`の付け外しだけで、両方とも描いておく**(タブを跨いでも入力中の値や
読み込み済みの一覧が消えない)。

**タブの見た目は`components/TabBar.tsx`の1か所**(両画面で共用。同じ見た目を2か所に
書くと片方だけ古くなる)。**選択中は下線で示す** —— 柱を切り替える操作なので、
押すたびに結果が変わるボタンの並びではなく、いまどの柱を見ているのかが読める形にする。
**横に溢れたらスクロールさせ、折り返さない**(タブの数と文字数は呼ぶ側で決まり、
「探す: 都道府県」のような見出しは狭い端末で1行に収まらない。折り返すと下線が途中で
切れて、どこまでがタブの並びなのか読めなくなる)。

**どちらの画面もタブの上に画面の名前を出す**(「スポット」「管理画面」)。タブだけが
並んでいると、そのタブが何を切り替えているのかが画面から読めない。スポット一覧の節の
見出しは`h2`に下げてある(画面の名前が`h1`になったため)。
...
- **「収集に直させる」は、収集を持っている種別にだけ出す**(行ごと・一括の2つ)。
  報告された理由をそのまま依頼文にして、**名指しで**収集へ割り込ませる
  (「割り込みは「じっくり」の枠で走らせる」)。CSVを直しに行かずにその場で頼めるので、
  収集で育てている種別ではこちらが片付けの本線になる。
  収集を持たない種別では直させる相手がいないのでボタン自体を出さない
...
### 周辺を探す(`/api/spots/nearby`・`/api/spots/discover`・`AiSpotDiscoverySearchModal`・`AiSpotDiscoveryPanel`)

地図の右クリック/長押しメニューの「この周辺を探す」(spot_admin/admin専用)。
検索語・半径・件数(10/20/30)と**探し方**を選ぶと、候補が地図に番号つきの印で並び、
右のパネルで選んでまとめて登録できる。

**探し方は選ばせない。まず地図データを引き、AIは任意の後段。**

| 段 | 口 | かかる時間 | やること |
|---|---|---|---|
| **地図データ**(必ず走る) | `GET /api/spots/nearby` | **2〜9秒**(半径による。下記) | 正確な名前と座標を漏らさず集める |
| **AIで精査・補完**(チェックしたときだけ) | `POST /api/spots/discover` | 17秒〜2分 | 一覧から選ぶ・ジャンルとランクを付ける・地図に無いものを足す |

**AIと地図データに別々のものを探させない。** かつては両方が独立に周辺を探していたが、
同じ店が二重に並ぶうえ、AIは「地図に載っている当たり前の店」を挙げるのに時間を使っていた。
**得意なことが違うので役割で分ける** —— 地図データは「在るもの」を全部並べるだけで
探しているものに合うかも記録する価値があるかも見ておらず(半径300mで1,800件)、
説明文も持たない。そこをAIに見せて**精査**させ、あわせて**地図データに無いもの**を足させる。
探索の画面のチェックボックス「AIに精査させる」を入れると、地図データを出したあとで
続けてAIに渡し、判定を一覧に当てて足りないぶんを同じ一覧に足す。
**既定は入れておく** —— 地図データは在るものを全部並べるだけなので、精査を通さないほうが
例外(「この辺に何があるか見たい」)になる。外したいときに外す形にしてある
(前回外していたら次も外れている。「もう一度探す」に前回の選択が渡るため)。

**AIは探索の画面ではなく`MapView`側で走らせる**(`runAiFollowUp`)。あちらは結果を出した
時点で閉じるので、中で待つと**出ている結果を見られないまま数十秒止まる**。
こちらなら地図とパネルを見ながら待てる。**待っていることはパネルに出す**
(`aiPending`)—— 出さないと「もう終わったのか、まだ来るのか」が分からない。

**地図データが引けなくても、AIで補うつもりなら止めない。** 日本以外の種別では地図データ
そのものが使えないので、そこで止めるとAIにも聞けなくなる。

**この順にしたのは「近くの飲食店をちょっと見たい」が主な使い方だから。** AIだけの頃は
1回に3分近くかかって使い物にならなかった。**まず地図データで雑に集め、要るときだけ
AIに精査させる**(足りなければパネルの「もう一度探す」。同じ一覧に混ざり、
行に出どころの印が付く)。

**どちらも同じ`DiscoveryResult`を返す**ので、地図の印もパネルも出どころを問わず
同じものを並べられる。違いは`source`(`map`/`ai`)だけで、画面はそれを印にする。

**出す条件は3つそろって初めて**(GET `/api/spots/discover?type=`が`enabled`を返し、
`MapView`はfalseならメニュー項目を出さない): ①接続先`CHIEZO_BASE_URL`がある
(どちらの段もchiezo経由。未設定なら機能そのものが出ない。IPはコードにもDBにも置かない)、
②種別の`ai_discovery_enabled`がon(**既定off**)、③呼んだ人がspot_admin/admin。

#### 1段目: 地図データ(`app/api/spots/nearby/route.ts`)

chiezoの地図辞典を引くだけ。**辞典を2つとも引いて混ぜる** ——
**Overture Places**(`overture_japan`。`lib/overtureNearby.ts`)と
**OSM**(`osm_japan`。`lib/osmNearby.ts`)で、**穴の位置が互いに違う**ため:

| | Overture | OSM |
|---|---|---|
| 日本の件数 | 301万 | 155万 |
| 飲食店(新宿500m) | **1,651件** | 403件 |
| 店のURL | **200件中193件** | 200件中51件 |
| 都道府県(`extra.area`) | **ほぼ空** | 名前が入る |
| 住所 | 自由記述(和英が混ざる) | `addr:*`で構造化 |

**店の数とURLはOverture、都道府県と構造化された住所はOSMが埋める。**
片方だけにすると、どちらかが必ず落ちる(`centerRegion`はたいていOSM側から取る)。

**辞典ごとに探し方を2つ投げる**:

- **全文検索**(`/search`)は「ラーメン」のように**名前に出る語**に強い。
  **同じものを指す書き方のぶん投げる**(`searchSpellings`)—— 全文検索は表記が違うと
  当たらない。片仮名で引くと、同じ語を平仮名で名乗る店が丸ごと落ちる(実測)。
  **3文字未満の言い換えは投げない**(chiezoの全文検索はそこでタイトルの前方一致に落ち、
  別物が混ざる)
- **種別での絞り込み**(`/filter`)は「ランチ」のように**名前に出ない語**に要る。
  片方だけだと、どちらかの語で何も出ない。**辞典ごとに引き方が違う**:
  - OSMは`feature=<主タグ>`。主タグ1つしか受け付けない
    (`cuisine=ramen`で絞ろうとすると0件。実測)ので、語→主タグの表を持つ。
    **料理の種類はこの形では絞れない**ので、`amenity=restaurant`のように広く取ってから
    `cuisine`タグでこちらが落とす(`cuisinesForWord`)。**タグの無い店は落とす** ——
    残すと「飲食店すべて」になって、種類を指定した意味が消える
  - Overtureは**検索語があるときは`tag=<カテゴリ>`**(カテゴリの並びのどこかに一致)。
    主カテゴリだけを見る`feature=category=`だと、**具体的な種別が副カテゴリにしか無い店が
    丸ごと落ちる**(実測: ある繁華街のbboxで`noodles_restaurant`は主55件に対し、
    並びのどこかなら127件。麺類の店の半分以上が出てこなかった)
- 全文検索の応答には座標が載らないので、当たった題名で`doc`を引き直す。**並列に投げる**
- **検索語が無いときもOverture側は既定のカテゴリで絞る**(`DEFAULT_OVERTURE_CATEGORIES`)。
  絞らないと、bboxに数千件ある中からATMや駐車場が先に並ぶ(新宿の小さなbboxで7,744件)。
  **ここだけは`feature=category=`のまま** —— 既定のカテゴリは`restaurant`のような
  大きいくくりを含み、`tag=`にすると件数が1.5倍に膨らむ(同じbboxで4,124件 → 6,172件)。
  何を探すか言われていないときは主カテゴリで代表させるほうが、読み切れる件数に収まる
- **ジャンルの名乗りは、地物が持つカテゴリのうち具体的なほうを採る**
  (`overtureGenreOf` / `genreOf`)。先頭から順に当てると、Overtureの
  `[japanese_restaurant, noodles_restaurant, …]`や OSMの`cuisine=japanese;ramen`が
  「和食」になり、**ラーメン店として探せなくなる**うえ、追加したスポットのカテゴリにも
  そのまま入る

**同じ店は両方に載っているので重ねてから返す。** Overtureを先に置き、OSM側は
正規化した名前が同じか、50m以内で名前が包含関係にあるものを捨てる ——
Overtureのほうが候補としての手掛かり(URL・電話)が多いため。

**Overtureのカテゴリ名は推測で書かない。** 存在しない名前を渡してもエラーにならず
0件が返るだけで、気づけない。`/v1/overture_japan/tags?contains=`で実在を確かめてから
表に足すこと。**日本の神社・寺は`church_cathedral`に入っている**(44,648件。
`shinto_shrines`は9件しか無く、そちらだけ見ると神社が丸ごと落ちる)——
ただし本物の教会も混ざるので、画面のジャンルは「寺社・教会」と言い切らない形にしてある。

**座標は地図データそのもの**なので`location_verified`は常にtrue。
**説明文は持たない**(どちらの辞典にも無いので`summary`はnull)。
**日本の種別だけ**(chiezoの辞典が日本の抽出)。

**件数の上限は持たない**(`UNLIMITED_DISCOVERY_LIMIT`)。ローカルのSQLiteを引くだけなので
絞る理由が無く、絞ると「この辺に何があるか」を見るという地図データ側の使い方ができない。
**件数を決めるのは半径**で、実測(新宿・既定のカテゴリ、2つの辞典の合計)はこうなる。
**既定はいちばん狭い300m** —— 広い側を既定にすると、開いた瞬間に読み切れない一覧が出る
(足りなければ「もう一度探す」で広げるほうが、多すぎる中から絞るより早い):
...
**chiezoの`filter`は1回500件まで**なので`offset`で継ぐが、**残りの頁は一度に投げる**
(`filterAll`)。1頁ずつ待っていた頃は半径1kmで11秒かかった —— 1回目の応答で`total`が
分かるので、何頁要るかはそこで決まる。**上限を持たないぶん、待ち時間は件数に比例する**。

**全文検索の側だけは上限が要る**(`SEARCH_DOC_LOOKUPS`)。`filter`は`extra`(座標・住所)を
一緒に返すが、**`search`は返さない**ので当たり1件につき`doc`を1回叩くことになる。
全文検索は関連の高い順に並ぶので、深追いしても得るものが少ない。

**件数の選択(`DISCOVERY_LIMIT_OPTIONS`。画面では「AIに任せる件数」)はAIに聞くときだけ出す。**
**精査させる地図データの件数**(中心から近い順にこの数だけ渡す)と**足させる候補の上限**を
兼ねる —— どちらも増やすほど答えが長くなって待ちが延びるので、つまみは1つにしてある。
待ち時間になるのはAIの側の事情で、地図データには当てはまらない(あちらは全件返す)。

**限界は測ってある**: どちらの辞典も開いたばかりの店は載らず、営業時間も持たない
(OSMで入っているのは新宿で23%・郡山で1%)。**だからAIの段を残してある。**

#### 2段目: AIで精査・補完(`app/api/spots/discover/route.ts`)

**AIに渡すのは1段目の候補**(`known`。名前・ジャンル・距離だけ。座標や住所を渡しても
使い道が無く、プロンプトが伸びたぶんだけ待たされる)。頼むのは2つ:

1. **精査** —— 一覧から「探しているものに合い、記録する価値があるもの」を**番号で**選び、
   ジャンルを付け直し、ランクと一言を付ける(`picked`)。番号にしてあるのは、
   店名を書き写させると一字一句が合わず突き合わせに緩さが要るうえ、答えが長くなるため
2. **補完** —— 一覧に無いものを足す(`added`)。従来のAI探索と同じ形で返り、
   同じように地図辞典で裏取りする

返すのは`reviews`(**渡した件数ぶん揃う**。選ばれなかったものも`picked: false`で入る)と
`candidates`(足したぶん)。**選ばれなかったことと渡していないことは別物**なので、
判定を欠番にしない —— 画面は前者のチェックを外し、後者はそのまま残す。
**渡した名前と同じものが`added`に混ざっていたら捨てる**(「一覧にあるものは足さない」と
頼んではいるが、守られないと同じ店が二重に並ぶ。ここで落とせば裏取りの往復も減る)。

**渡すのは近い順に「AIに任せる件数」まで**(登録済みのものは除く。追加できないので
精査しても使い道が無い)。地図データは半径の中を全部返すので、そのまま渡せば
プロンプトが読み切れない長さになる。

**実測**(antigravity+low、地図データ10件を渡して精査+補完): **23秒**で、10件のうち4件を選び
(駐車場と半径の外の1件は落ちた)、ジャンルを付け直し、ランクと一言を付け、一覧に無い3件を
足した。**探させるだけだった頃と同じ桁**(17秒)に収まっている —— 一覧を読ませるぶんは
増えるが、選ぶ側の答えは番号1つで済むため。

**答えの読み取りは形を守らなくても通す**(`extractAnswer`)。`{"picked":…,"added":…}`が
本来だが、配列だけを返してくることがあるので、そのときは要素の中身で振り分ける
(`no`を持てば精査、名前と座標を持てば足したぶん)。**この機能は1往復しか無く、
言い直せる相手がいない**ので、返ってきたものを読めるほうが実用的。

**検索語は無くてよい**(1段目と同じ)。「この辺に何があるか地図データで見て、
記録する価値のあるものをAIに選ばせる」使い方では入れる語が無い。

**その場で答えが欲しい機能なので、正確さより速さを採る。** 実測で効いたのは3つ:

| 変えたところ | 理由 |
|---|---|
| **検索の回数に上限を言い渡す**(`quick`) | **これが一番効いた。41秒→17秒**(同じ相手・同じ件数)。エージェントとして動く相手は、放っておくと候補ごとに裏取りの検索を回す。「webの検索は多くても2回」と書き、URLも必須にしないだけで半分以下になり、**遅い側のばらつきも収まる**。裏取りが要るときは画面で「しっかり」を選ぶ |
| **プロンプトを短くし、頼む項目を最小限にする** | 項目を1つ増やすと候補の数だけ文が増える。`region`(中心座標から引ける)・`name_kana`・`rank_reason`は落とし、`rank`は**その種別がランクを使うときだけ**聞く |
| **モデルとeffortを選べるようにする** | 同じ問いで **32秒〜3分半** の開きが出た(実測・単発: antigravity+low 32秒で10件 / haiku+low 64秒 / codex 100秒 / sonnet+medium 215秒)。未選択なら`low`(`DEFAULT_DISCOVERY_EFFORT`。**相手がeffortを持つときだけ送る** —— 持たない相手=codexに送るとchiezoが400で断る) |
| **位置の確認を候補ごとに直列でNominatimに聞くのをやめる** | 1秒1回の間隔制限がそのまま待ちになり、10件で10秒以上。しかもchiezoに無い店(=新店)は同じOSM由来のNominatimにも無いことが多く、待った割に当たらなかった |

**相手・モデル・深さは探索の画面で選ぶ**(「AIの設定」。AIを選んだときだけ出る折り畳み)。
**管理画面に置かない**のは、その場の目的で軽くも重くもしたい設定だから —— 全員ぶんを
1つに決めさせると使う人が選び直せない。**選択はサーバーに保存せず、その端末の
localStorageに覚える**(**DBに列を持たせない**判断でもある —— 種別ごとのon/offと違い、
これは人ごと・回ごとに変わる値)。選ばなければ`CHIEZO_AI_BACKEND`か既定に落ちる。
選択肢はGET `/api/spots/discover?type=`が配る(**web検索を持つ相手だけ**)。POSTは
`resolveDiscoveryChoice`で確かめてから投げるが、**相手の一覧が取れなければ素通しする**
(確かめられないことを理由に探索そのものを止めない)。

**精査でAIが上書きするのは言葉だけ**(ジャンル・要約・ランク)。名前と座標は辞典のほうが
正確なので触らせない。**足したぶんについても同じ** ——
**AIの答えは信用せず、事実は地図データから取り直す。** 候補ぶん並列にchiezoの地図辞典
(`overture_japan`→`osm_japan`の順。AIが挙げるのは新しい店・話題の店が多く、
そこがいちばんOSMの穴と重なるためOvertureを先に見る)を引き、当たった候補は
**座標・住所・公式サイトを地図側の値で置き換える**。AIに残るのは「どの店を挙げるか」と
要約だけになる。重複は名前一致か50m以内で「登録済み」。**口コミ本文・点数・
ランキング順位の転記はプロンプトで禁じ**、要約はAI自身の言葉に限る。
地域はAIに聞かず中心座標から引いたものを全候補に使う。

**引くときは問い合わせ語を段階的に緩める**(`mapLookupQueries`)。**AIの店名は地図の
見出しと一字一句は合わない** —— 生の名前だけで引いていた頃は`アカシア 新宿本店`が
地図の`アカシア本店`に当たらず、`ベルク (BERG)`も`ベルク`に当たらなかった。
括弧の中(英字の別名)→ 支店の名乗り(`〜本店``〜新宿店`)→ いちばん長い語、の順で
落としていき、**当たったらそこで止める**(緩めるほど別の店を掴む危険が上がるため)。
緩めたぶんの歯止めが`looksLikeSameSpot`で、正規化した名前が互いを含むことを求める
——地図側は`天ぷら新宿つな八総本店`のように業種や地名を前に付けるので、完全一致では狭すぎる。

**確かめられなかった候補は「地図データに無い」と言い切る。** かつては「位置未確認」と
出していたが、あれは位置の話に見えて実際には**その店が在るかどうか**の話だった。
**最初から選んでおくかの判定もここに変えた** —— 以前は参照URLの有無で決めていたが、
URLはAIが書いた文字列でしかなく確かめる相手がいない。しかも実測で`quick`のAIは
**URLを1件も返さない**一方、地図側は10件中9件で公式サイトを持っていた ——
根拠が無いように見えていた候補のほとんどに、実は出どころがあった。

**半径の外に出た候補は行に印を出す**(`DiscoveryRow.radius`。「もう一度探す」で別の
半径の結果が同じ一覧に混ざるので、**行ごとに**頼んだ半径を持つ)。頼んだ範囲を無視して
数を合わせにくることがあり、地図で見るまで気づけなかった(実測で1件、700m先が混ざった)。

**`quick`のプロンプトから「知っている店をそのまま挙げてよい」を外した。** 速さを買って
いたのは検索回数の上限のほうで、この1行は記憶からの捏造を明示的に許していた。
代わりに「実在すると確信できる店だけ」「半径の外は入れない」「件数が足りなくてもよい」を書く。

**待つ上限は180秒**(`CHIEZO_AI_TIMEOUT_SECONDS`)。**待たされた末に失敗するより、
早く諦めて条件を変えて出し直せるほうがよい**。失敗の文言には**必ず相手の名前を入れる**
—— どの相手が失敗したのか分からないと切り替えの判断ができない。

**投げた本文と返ってきた本文は結果に載せ、画面から見られるようにしてある**
(`DiscoveryExchange`・`AiExchangeDialog`。パネル見出しの「AIとのやり取り」)。
**手を入れずに出す** —— 整形すると直したいプロンプトの実物と食い違う。
遅い・少ない・的外れの原因は、プロンプトなのか相手なのか件数なのか、ここを見ないと
切り分けられない。**JSONとして読み取れなかったときはエラー文にも本文の頭を載せる**
(そのときこそ何が返ったか見たい)。

**相手によっては同時に1本しか動かない。** chiezoのブリッジは認証情報がホーム配下で回る
CLI(codex / antigravity)の起動を直列にするので、2人が同時に探すと後の人は待つ
(実測: 3本同時で1本目238秒、残りは600秒待っても返らなかった。1本ずつなら32秒)。

#### 画面と登録

候補は番号つきの印で、**色は探し方で分ける**(地図データは青`#0284c7`、AIは紫`#7c3aed`、
登録済みは灰色。チェックを外したものは薄く、押している行は青い縁)。同じ一覧に両方の候補が
混ざるので、地図の側でも見分けられないと「これはAIが言っているだけの店か」が分からない。
**パネル側の番号バッジと出どころバッジにも同じ色を塗る** —— 片方だけ変えると一覧と地図の
対応が切れる。かつてパネルの番号は出どころを問わず紫で、**地図で青い印を押しても一覧の
番号が紫で出る**ため、色から対応を読めなかった。
**一覧の行を薄くするのは「登録済み」「地図データで実在を確かめられなかった」
「AIが見たうえで選ばなかった」の3つだけ**。
かつては参照URLの無い行を薄くしていたが、URLはAIが書いた文字列でしかなく確かめる相手が
いない。しかも地図データ側の候補はURLを持たないものが多く(OSMは実測で200件中51件)、
**意味の無い理由で行の大半が薄くなっていた**。印の色分け・最初から選んでおくかの判定と
同じ根拠にそろえてある。
**精査させるときは地図データの行を選んでおかない**(`appendDiscoveryResult`)——
選ぶのはAIの仕事になるため。半径の中の全件を選んだままにすると、精査した数件と
精査していない千件が混ざったまま「1,792件を追加」という選択になってしまう。
精査させないときは従来どおり最初から選んでおく(出どころが確かなため)。

**精査の結果は行の印で出す**(「AIが選定」は紫、「AIは選ばず」は灰色)。地図データの行に
ジャンル・ランク・一言が付いているのは精査を通ったからで、断りが無いと辞典が持っていた値に
見える。**選ばれなかった行も消さない** —— AIも見落とすので、チェックを外して薄く出すに留め、
選び直せるようにする(渡していない行は印が付かず、そのまま残る)。

**狭い画面では右の帯ではなく下から敷く帯にする**(`top-1/2`で高さ50%、`sm:`から
従来の`right-0 w-2/5 max-w-sm`)。右の帯のままだとスマホの幅では150px前後しか残らず、
**行の中身が1文字ずつ縦に折り返されて読めない**。
**高さは半分まで**にして上半分の地図を必ず残す —— 候補を押して位置を確かめるのが
このパネルの使い方なので、地図が3割しか見えないと確かめる先が無い。
そのぶん上下の操作を詰めてある: 見出しの説明文は狭い画面では伏せ、
**「AIとのやり取り」は見出しの右**へ、**状態のセレクトは見出しを付けず追加ボタンと同じ行**へ
置く(どちらも下の操作から1行ぶんずつ減る)。

**寄せ先は地図の中心ではなく、パネルに隠れていない側の中心**。効かせるのは2か所で、
**どちらも同じ`visibleCenterOffset`**を使う:

- **パネルが開いた(探し直しで中心が変わった)とき**、探した中心をそこへ寄せる。
  右クリックした点はそのとき地図の真ん中とは限らないうえ、パネルが画面の半分を覆うので、
  **開いた直後は探した中心も候補の半分もパネルの下**に入っていた。**縮尺は動かさない**
  (探した範囲の見え方を勝手に変えない)
...
——**折り返しは要素ごと**(`whitespace-nowrap`)にして、文字の途中では切らない。

**行を押すとそのピンへ寄り、ピンを押すとその行が目立つ**。パネルでできるのは、
チェック・**見出しの「すべて選ぶ / すべて外す」**(登録済みの行は対象外)・
×(候補から外す)・行ごとのランクとカテゴリ・「もう一度探す」(**同じ一覧に足す**。
正規化した名前が同じ候補は二度入れない)・「追加」・「終了」。

**最初のチェックは自動で決まるので、まとめて選び直す口が要る。** 精査させたときは
AIが選んだ行だけ、AIが足した行は地図データで実在を確かめられたものだけが入った状態で
始まる —— その判断ごと引き受けたい・捨てたいときに、1件ずつ押させない。

**通し番号は`setDiscoveryRows`の更新関数の中で`prev`から決める**(`max(no)+1`)。
**カウンタを`useRef`で持って`++`してはいけない** —— それは更新関数の中の副作用で、
StrictModeが更新関数を2回呼ぶ開発時には番号が2倍消費される(1回目で1〜Nを配って
捨てられ、残る2回目がN+1から始まるので、**1件目の番号が件数ぶんずれる**)。
上限を外して地図データが数千件返すようになったとき、1799番から始まって表面化した。

**印はDOMのマーカーではなく地図レイヤーで描く**(`DISCOVERY_SOURCE_ID`)。地図データ側は
件数の上限を持たず数千件になるので、1件1要素だと**地図を動かすたびに全部を置き直す**
ことになって固まる(MapLibreのMarkerは移動のたびに位置を計算し直す)。
**重ね表示より更に上へ移す**(`moveOverlayLayersToTop`の末尾)—— 探している間はそれが
主役で、半透明の別種別に隠れると番号が読めない。

**探す範囲を薄い青円で出す**(`DISCOVERY_RANGE_SOURCE_ID`・`circlePolygon`)。
**この円は2つの取り違えで消えていた。** どちらも「探し方の画面へ渡す関数」の扱いで、
同じ形の配線を足すときは踏みやすい:

- **半径を知らせる関数は`useCallback`で固定し、同じ半径なら state をそのまま返す**
  (`handleDiscoveryRadiusChange`)。毎回新しいオブジェクトを作って渡すと、
  「円のstateが変わる → 地図が描き直される → 渡す関数の同一性が変わる →
  向こうの`useEffect`(依存に関数が入っている)がまた知らせてくる」で更新が止まらず、
  Reactが`Maximum update depth exceeded`で描画を打ち切る。**打ち切られると円が出ないだけ
  でなく、探した結果のパネルも更新されない**ので「何も見つからなかった」ように見える
- **閉じるときの判定はstateではなく`discoveryPanelOpenRef`から読む。**
  探し方の画面は「結果を渡す → 自分を閉じる」を続けて呼ぶので、`onClose`が読む
  `discoveryPanelOpen`は**パネルを開く前の値のまま**。それで判定すると
  「探さずに閉じた」と誤解して、**探した直後に必ず円が消えていた**。
  refは`useEffect`任せにせず、`appendDiscoveryResult`/`closeDiscovery`で
  stateと同時に手で立てる(effectはコミット後にしか走らないため間に合わない)

**探し方を選んでいる間から出す**ので、半径を選び直すと即座に変わる(どこまで探すのかを
決める手掛かりになる)。結果を見ている間も出したまま、パネルを閉じると消える。
**`circle-radius`は使えない** —— あれは画面のピクセルなので拡大縮小しても大きさが
変わらず、「半径500m」を表せない。地面に貼り付いた円が要るのでポリゴンを組む。

**探している間は左クリックでもメニューが出て、中身は「ここでもう一度探す」だけになる。**
探した後に「隣の区画も見たい」が続けて起きるので、そのたびに右クリック(スマホなら
長押し)を強いるのは重い。**ピン・候補の印を押したときは出さない**(そちらの操作が先)。
地図のクリックハンドラは一度しか束縛されないので、いまパネルが開いているかは
`discoveryPanelOpenRef`から読む。

**各行から「座標と店名を見比べる」でGoogle マップを開ける**(`buildGoogleMapsCompareUrl`)。
**経路検索(`dir`)の出発地に座標・目的地に店名**を入れる形で、1枚の地図に両方の印が出る
——検索(`search`)は問い合わせを1つしか受け取れないので、2地点を同時に出すにはこれになる。
**副産物としてずれが距離で読める**(同じ場所なら数十m、違う場所なら数百m〜数km)ので、
目で見比べるより判断が速い。`travelmode=walking`にしてあるのは短い距離をメートルで
出すため(車だと一方通行の遠回りが混ざって、ずれの大きさとして読めない)。
**目的地の側に座標を足さない** —— 足すとGoogle側がその座標に寄せて解決するので、
間違った座標でも「合っている」ように見えて確かめる意味が無くなる。

**探索の結果は保存しない。** 追加したものだけが通常のPOST `/api/spots`(`origin='manual'`)で
入る。説明文は`buildDiscoveredDescription`が組み、**出どころを必ず添える**
(地図データなら「Overture Mapsから取得(日付)」「OpenStreetMapから取得(日付)」、
精査を通っていれば「ジャンルと要約はAIが精査」を足す(`ai_reviewed`)——
...
**CDLA Permissive 2.0 / Apache 2.0**(継承条件なし)。この機能が引くのは
**自分のインスタンスで使うため**で配布ではない(地図表示には
`© OpenStreetMap contributors`を出している)。ただし`origin='manual'`は
還元用エクスポートの抽出条件なので、**OSM由来の名前・座標を
travel-log-data(MIT側)へ持ち込まないよう、還元するときは出どころを見て外すこと**
(「外部データソース…注意」の「ヒントに留める」と同じ線)。**説明文に辞典名を
書いてあるのはこの判断のため** —— 混ぜて取り込むので、どちらから来たかが
後から読めないと外しようがない。Overture由来のぶんは継承条件が無いので、
同じ扱いで外す必要はない。

### 情報を集めさせる(`/api/spots/collect`・`lib/spotCollect.ts`)

**周辺を探すの、待たない版**(spot_admin/admin専用)。知識サーバー(chiezo)の
「収集」へ**依頼だけして離れ**、集まった頃に取り出す。あちらはその場で探して数十秒で
返すので、探せる範囲も精度も1回の待ち時間で頭打ちになる —— こちらは待たない代わりに
時間を掛けさせられるので、地図に載っていない新しい店のようなものを拾える。

**溜め先はスポット種別ごとに1つ**(chiezoの収集 = 1ソース)。名前は種別キーから作り
(`tl_<key>`。あちらの制約は英小文字で始まる2〜31文字)、実際に使う名前は設定に持つ
(`collect_source`)—— 種別キーを変えても、いちど依頼した収集を指し続けられるように。
**こちらが持つ設定はこれだけ**。プロンプトが2本・抽出条件・区画と増えたので、
写すとどちらが正か決められなくなる —— **正はchiezo側**。
...
| 口 | やること |
|---|---|
| `GET /api/spots/collect?type=` | 状態(収集・区画数・巡回ごとの一周の進み具合・取り込みの状態) |
| `POST /api/spots/collect?type=` | 依頼する(抽出条件+区画+巡回を組んで`POST`/`PATCH`) |
| `POST /api/spots/collect/draft?type=&kind=extract\|prompt` | 抽出条件・プロンプトをAIに書かせる(**保存しない**) |
| `POST /api/spots/collect/run?type=` | 予定を待たずに1回(`sweep`で巡回を名指し / `partition`・`titles`で割り込み) |
| `GET /api/spots/collect/candidates?type=` | 溜まったものからスポットの候補を取り出す |

#### 区画(bbox)で回す

**1本のカーソルでは「一周した」が言えない。** かつては`{cursor}`に次の市区町村を
AIに書かせ、`{covered}`に回り終えた地名を積む形だったが、取りこぼしがどこかも、
全国を舐め終わったかも分からなかった(全国約1,700市区町村を数時間に1つでは200日を超える)。
しかも**収集済みの判定がAIの書いた地名の表記に依存していて**、「東京都新宿区」と
「新宿区」の揺れを吸う処理と、円周8方位を突いて市区町村を引く処理が要った。

いまは**区画**で回す(`partition: {by: "geo", source, tag, bbox, target}`)。chiezoが
対象の空間を矩形に割り、巡回が順に配る —— 区画は数え上げられるので「一周した」が言え、
進み具合も矩形なので表記の入り込む余地が無い。

- **割るのは対象としている空間であって、集まったものではない。** 母集団は地図辞典
  (`overture_japan`・`osm_japan`)を指す —— こちらが1件も持っていなくても、あちらは
  北海道にも店があることを知っている。**そもそも収集の文書は座標を持てない**
  (chiezoの`extract`は元の`extra`を運ばない)ので、自分の点では割れない
- **bboxは日本全体を明示する**(`JAPAN_BBOX`)。書かないと母集団の外接矩形になり、
  「まだ何も無い範囲」が対象から落ちる
- **点の数が`target`を切るまで再帰的に二分する**(k-d treeと同じ割り方)ので、
  都市部は細かく地方は粗く、かつ全域に区画がある
- **`target`は母集団の件数から逆算する**(`partitionTarget`)。chiezoは1回に見る区画を
  20までに制限するので、6時間ごと3日=12回なら240区画が上限 —— 区画数を`PARTITION_GOAL`
  (200)あたりに保たないと、一周が静かに延びる。件数が分からないときは書かない
  (0件として逆算すると最小の10になり、区画が膨れ上がる)
- **区画はタグでしか絞れない**(chiezo側は`tag`を1つだけ受け取る)。抽出条件の
  カンマ区切りの先頭を使い、**タグの無い抽出条件は投げる前にこちらで断る** ——
  ソース全体は50万点の上限を超えて409になる
- **区画は依頼した時点では割られない**(最初に走ったときに割られ、以後は必要なら割り直す)。
  画面の「区画: N」は0のあいだ出さない
...
| 巡回 | 時計 | 設定 | やること |
|---|---|---|---|
| 名簿 | 週1 | `use_extract` + `only_new` | 抽出条件で地図辞典から名簿を引き直す(**AIを呼ばない**) |
| ざっと | 6時間 | `cover_days: 3` | 全区画を舐めて精査・整理する |
| じっくり | 12時間 | `cover_days: 7` + `after: "ざっと"` | 1区画ずつ詳しく調べる |

- **名簿は`only_new`と組にする。** 組にしないと、AIが肉付けしたぶんを名簿の薄い内容
  (名前と所在地だけ)で上書きする。**週1で回すのは、地図辞典に店が増えるから** ——
  機械で埋まるのが「1回目だけ」だと、そのあと増えたぶんは永遠に入らない
- **じっくりはざっとが一周するまで走らない**(`after`)。名簿を作った直後に詳細を
  聞いても「もう入っているもの」を並べて終わる —— 区画の中身が一度行き渡っていないと、
  何が足りないのかを判断する材料が無い
- **一周の日数は`cover_days`で書く。** 1回に見る区画数を手で書くと、区画が増えた日に
  一周が静かに伸びる(そして誰も気づかない)
...
頼み方は2つあり、**どちらも`POST /v1/collect/{name}/focus`**(`./run`の口が振り分ける):

| 渡すもの | 入口 | 使いどころ |
|---|---|---|
| `titles` | 管理画面の「⚠ 間違い報告のあったスポット」 | **この1件を直して** |
| `partition` | 地図の「この範囲をいま集める」 | **この範囲を先に見て** |

- **名指しできることが要**。区画を渡すだけでは、直してほしい1件が差し込みに載る
  保証がない(その区画の中身が多ければ途中で切られる)。1回に渡せるのは
  `MAX_FOCUS_TITLES`(20)件まで —— それ以上は差し込みに載り切らない
- **`run`ではなく`focus`で頼む。** `run`は予定も区画の巡回記録も進めるので、
  割り込むたびに一周が伸びたり、見ていない区画に印が付いたりする。`focus`は
  そこを動かさない(動くのは中身だけ)
- **`note`が無いと向こうが受け付けない** —— 何をどう直すかが書かれていない割り込みは、
  1回ぶんのAIの呼び出しにしかならない(巡回でやれば済む)。間違い報告から頼むときは、
  **報告された理由をそのまま依頼文にする**(「テキストで表示」と同じ`- 名前: 理由`の形)
- **`after`は割り込みには効かない**(定時の`is_due`だけを縛る)ので、ざっとが
  一周する前でも名指しで直させられる

#### 抽出条件とプロンプトはAIが書く

**集めたい軸は種別ごとに違う**(ラーメン店・神社・駅・道の駅…)ので、どのソースの
どのタグを引くかも、どう調べさせるかも決め打ちにできない。`./draft`に**ふつうの言葉**の
依頼文を渡して書かせ、確かめてから依頼する。

- **`kind=extract`は`/v1/collect/draft-extract`** —— 抽出条件を書いて、
  **その場で引いた件数と先頭数件を添えて返す**。**タグは完全一致でしか引けない**ので、
  それらしい名前を書かれると静かな0件になる(0件のときchiezoは実在するタグ名を候補として
  返すので、画面はそれを出す)。この条件は**区画の母集団にも使う**
- **`kind=prompt`は`/v1/collect/draft`** —— 返させるJSONの形・`{partition}`の使い方・
  titleが重複の鍵であることはchiezoがsystemで教えるので、渡すのは「何を集めたいか」だけ
- **2つに分けてあるのは、1回ずつ待たせるため。** どちらもAIが1回動くので十数秒〜数分
  かかる。まとめて走らせると待ち時間が積み上がるうえ、**抽出条件が決まってから書いた
  プロンプトのほうが良くなる**(どの辞典から引いた名簿を精査するのかを書けるため)
- **「じっくり」のプロンプトだけはこちらが持つ**(`defaultDeepPrompt`)。種別ごとに
  違うのは**何を集めるか**であって**どう疑うか**ではない —— 確かめてほしいのは
  「実在するか」「所在地は合っているか」「二重に入っていないか」で、これは
  ラーメン店でも神社でも駅でも同じ。AIの呼び出しが1回減るぶん、依頼までの待ちも短い
- **どう回すか(週1の名簿・3日で一周・7日で一周)はアプリの決め事**なので、AIにも
  画面にも預けない —— 種別ごとにばらつくと、進み具合の読み方も種別ごとに変わる

#### 取り出し(`./candidates`)

**取り出しの位置(カーソル)は覚えない。** 毎回いちばん新しいものから
`COLLECT_FETCH_LIMIT`(50)件を取り、既に追加したものは「登録済み」の印で分かるようにする
—— 覚えると、画面を閉じただけ・別の端末で開いただけで取りこぼす。

読むのは`GET /v1/<name>/recent`で、**返る鍵は`docs`**(`search`/`filter`の`results`とは
違う。実測で確認)。**`fields`で`body`まで取る** —— 既定の`opening`は1段落に均されていて
改行が落ちるが、所在地を行として書かせているので本文が要る。

**座標はAIに書かせず地図辞典から引き直す**(周辺を探すと同じ流儀)。集めた文書にあるのは
名前と所在地までで、そこから`overture_japan`→`osm_japan`を名前で引く。ここが周辺を探すより
難しいのは**bboxで絞れない**ことで(中心が無い)、歯止めを2つ掛ける:

1. `looksLikeSameSpot`(正規化した名前が互いを含む)
2. **所在地との突き合わせ**(`matchesArea`)。行政単位で切った地名(`areaWords`)が、
   地物の`area`・`locality`・住所のどれかに出てくること。
   **「東京都新宿区」は1語で書かれる**ので、末尾の「区」だけを落とすと「東京都新宿」になり
   どこにも現れない(実測で、辞典に在る店が丸ごと落ちた)—— 単位ごとに切って
   「東京」「新宿」にすれば当たる。**Overtureは`area`がほぼ空**なので、
   `locality`(市区町村)を見ないと突き合わせる材料が無い

**確かめられないときは当てない**(所在地が読めない・地物側に所在地が無い)。名前が同じ
だけの別の場所を掴むと座標もURLも黙って別のものに置き換わる(実測: 「新宿御苑」で引いた
1件目が世田谷区の同名の地物だった)—— 取りこぼすより悪い。当てなかった候補は
**座標なしのまま残し**(`location_verified: false`、`lat`/`lng`は0)、画面が
「座標なし(位置を直す)」の印を出してチェックできないようにする。住所からは引き直せるので、
...
**判定は区画の巡回記録で行う**(`partitionCoverage`)。円に重なる区画のうち「ざっと」が
一度でも見たものを数え、まだのものは鍵をそのまま返す。**重なりは矩形と円の距離で見る**
(円の外接矩形との重なりでは、角のぶんを収集済みに数えてしまう)。

**まだの区画は「この範囲をいま集める」で先に見させる**(`?partition=` → `focus`。
走る枠は「じっくり」)。**1つずつしか頼めない**(あちらは同時に1本しか走らせない)ので、
先頭を渡す。

**中心の地域は1点だけ引く**(`areaAt`)。**座標を引き当てられなかった候補**が円の中の
話かを見るのに要る —— 新しい店は地図辞典に載っていないのが普通で、そこを落とすと
この機能で拾いたいものがちょうど落ちる。**円が跨ぐ隣まで拾う必要があったのは
収集済みの判定のほう**で、そちらは区画が答えるようになった。
...
こちらが答えるのは「この辺りは収集済みか」で、収集の単位は区画。

**管理画面は①抽出条件→②ざっとのプロンプト→③じっくりのプロンプト(既定のまま使える)
→依頼する、の順**に並べる。①②はそれぞれAIに書かせるボタンを持ち、**押すたびに
待たされる**(数十秒〜数分)。あわせて**巡回ごとの一周の進み具合**(`N/M区画`)と
**いま収集中か**(`GET /v1/ingest/status`)を出す —— 知識サーバーは同時に1本しか
受けないので、走っている間に頼んでも断られる。**「いま1回」は巡回ごとに押せる**
(相手も1回に見る量も巡回ごとに違うので、名指しできないと分けて持った意味が半分になる)。

出てくるのは同じ`DiscoveryResult`なので、地図の印・行ごとのシリーズ・登録済みの印・
まとめて追加がそのまま使える —— 出どころが違うだけのものに別の画面を作る理由が無い。
**出どころは3つ目**として`DiscoverySource`に`collect`を足し、地図の印もバッジも
専用の色(teal)にする。**中心が無いので探す範囲の円は出さず、距離も出さない**。
代わりに、取り出したあと座標の引けた候補全体が入るよう地図を寄せる。
...
**訪問予定の★は見出しの左端(バッジの左)に置く。** **×の隣に置かない** ——
指で押す端末では、閉じるつもりが訪問予定に入る誤爆が起きる。行の反対側の端なら、
押し間違えても被害が無い(右端は×だけにする)。

**スポット名はその場のアイコンからコピーできる**(`CopyTextButton`)。置き場は
**名前の最後の文字のすぐ右**(`h2`の中。上端ぞろえなので「右上」に見える)——
右上のボタン(×)へ寄せると、何をコピーするのかが名前と結びつかない。
...
**訪問記録フォームは「保存ボタンが画面内に収まる」高さを保つ**(モーダルは`max-h-[85dvh]`でスクロールできるが、記録するたびにスクロールさせるのは日常操作として重い)。そのため**説明文は畳んで置く**: 未訪問記録の説明は見出し横の`HelpTip`(下記)、口コミは既定で畳んだ入力欄にしてある。**`HelpTip`のボタンは`label`の外に置くこと**(中に入れると押したときにチェックボックスまで切り替わる)。口コミは**畳んでも入力済みの本文はそのまま投稿する**(`AddSpotModal`の「訪問を記録」と違い、開閉は意思表示ではない)ため、畳んだ見出しに「入力済み」と添えて分かるようにしてある。

`visits.visited_on`(timestamptz、nullable)は訪問した日時で、未入力なら`null`=表示は「時期不明」(`formatVisitedOn`)。入力は`datetime-local`のため常にローカル時刻で、送信時にISO 8601(UTC)へ変換してから渡す(文字列のまま送るとDB側がサーバーのタイムゾーンで解釈してずれる)。訪問記録フォーム(`VisitFormModal`)で写真を選ぶと、その写真のExif撮影日時を訪問日時欄に入れるボタンが出る(自動では入れない。複数枚選んだ場合は最も古い撮影日時)。Exifの読み取りは依存を増やさず`lib/exif.ts`の自前実装(JPEGのAPP1から`DateTimeOriginal`、無ければIFD0の`DateTime`だけを読む)で、縮小前の元ファイルから読む — canvasで描き直した時点でExifは失われるため。**保存済みの写真からは読めない**のもこれが理由で(サーバーにあるのは縮小後のJPEG)、記録をあとから開いて日時を直したいときのために、訪問日時欄の下に**写真を添付せず日時だけ読む「写真から読む」**を別に置いてある(`VisitFields`の`handleReadTakenAt`)。**こちらは確認のボタンを挟まずそのまま入れる** —— 利用者がそのために選んだファイルなので、「入れるかどうか」を改めて聞く意味が無い(添付した写真から読むほうは、写真を足すこと自体が目的なのでボタンのまま)。日時が入っていない写真を選んだときは、黙って何もせず断りを出す。Exifの日時にはタイムゾーンが無いので端末のローカル時刻として解釈する(datetime-localの扱いと揃う)。Exifが無い・JPEG以外(HEICなど)・壊れている場合は例外を投げずボタンを出さないだけにする。
...
複数スポットを順序付きでまとめる「訪問予定リスト」(旅程)。1スポットごとの`visit_plans`(行きたい場所のブックマーク)とは**独立**で、`/[type]/spots`の訪問予定欄に個別の予定スポットと**混じって**並ぶ(見出しは0件でも常に表示する)。スキーマは`visit_plan_lists`(種別ごと=`spot_type_id`、`title`・`description`・`start_date`・`end_date`(単日は開始=終了。**どちらもnullなら「訪問日未定」**)・`user_id`)+`visit_plan_list_items`(`list_id`・`spot_id`・`seq`・`visited_at`、`(list_id, spot_id)`一意)の2テーブル(`db/init/01_schema.sql`、移行は`migrations/006`・`008`・`016`)。**種別ごと**(地図の作成が`/[type]/map`上で行われるため。CLAUDE作成時の判断で種別横断は不可)。
...
**一覧の並びは開始日の昇順で、訪問日未定は末尾**(同じ開始日なら作った順)。これから回る順に読めるようにするため。アーカイブの一覧だけは新しくしまった順(最近しまったものから探すため)。

**訪問日は「未定」にできる**(`start_date`・`end_date`がnull)。行き先だけ先に決めて日取りは後から、という旅程のため。**開始日と終了日はセットで持ち、片方だけの状態は作らせない** —— DBのcheck制約(`visit_plan_lists_dates_ck`)と、作成・更新のAPIで共用する正規化(`lib/visitPlanListDates.ts`。終了日だけの指定は400、終了日が空なら開始日と同じ)の両方で塞いである。「開始日なしの終了日」は期間として意味を持たないうえ、一覧の並び順も天気の基準日もどちらを見るかで変わってしまう。入口は基本情報モーダルの「訪問日は未定」で、チェックすると日付欄が無効になり、外せば今日が入る。**画面の表記は「訪問日未定」**(`formatPlanDateRange`)—— 空欄にすると、日付を消し忘れたのか決めていないのかが読み手に伝わらない。
...
- **各スポットの右の天気**(`WeatherAskLink`)は、**その日の予報のアイコン**を出し、押すと
  **そのスポットの「予定の日」の天気をAIに聞く**(`lib/weather.ts`)。
  聞く日は**開始日→終了日**の順で決め、**どちらも無い(訪問日未定)ときは天気を出さない**
  (`planWeatherDate`がnullを返し、各行の天気も「前後1週間で天気の良い日を探す」も消える)。
  **今日で代用しない** —— 日が決まっていないのに今日の予報を出すと、今日行くものと読める。
  **天気サービスのページは日付を指定して開けない**
...
- **1日ぶんを1行にたたむときは、いちばん悪い地点に合わせる**(`summarizeDay`)。
  ...
  「晴れているが雨の可能性が高い」は言い切らずに間へ置く(境目は下記`POP_HIGH`と共有する)
- **上流の降水確率は数値で出さない**(`precipChanceText`で「雨の可能性 低/中/高」に丸める)。
  `precipitation_probability_max`は**気象庁が発表する降水確率とは別物** ——
  あちらは「6時間区切りで1mm以上」を10%刻みにしたもの、こちらは「時間ごとの降水確率
  (閾値0.1mm相当)の、その日の最大値」で、**定義が違うぶん構造的に高く出る**。
  数値のまま並べると国内の予報と突き合わせられ、同じものだと誤解される。
  段階の境目(`POP_MID`=40・`POP_HIGH`=70)が高めなのはこの差を見込んでいるため
- **`FAR_FORECAST_DAYS`(5日)より先の予報は言い切らない。** そこから先はモデルごとに解が
  割れる(気象庁の週間予報でも信頼度の低い日が並ぶ範囲)ので、`WeatherAskLink`はアイコンを、
  `PlanWeatherFinder`は行の天気部分を薄く出す。**消しはしない** ——
  気温のように先でも揃う要素は服装の見当を付ける役に立つ。
  判定の`isFarForecast`は**JSTの今日**から数える(`todayKeyJst`)——
...
**回る順番は、作成モードに入らなくてもその場で入れ替えられる**。並び替えの操作(左端の三本線ハンドルをつかんでドラッグ)は`lib/useDragReorder.ts`に切り出してあり、**4か所で共用する** —— 作成モードの`PlanBuildPanel`・リスト詳細(`VisitPlanListDetailModal`)・地図の経路詳細(`MapView`の`routeDetailView`。訪問予定リストのときだけハンドルを出す)・管理画面のスポット種別の並び替え(`AdminView`)。`touch-action: none`はハンドルにだけ当てる(`REORDER_HANDLE_CLASS`。行全体に当てると一覧そのものがタッチスクロールできなくなる)。

**この共通処理には、指の速さで結果が変わる踏み方が2つある。** どちらも「Reactの描き直しを待たずに次の`pointermove`が来る」ことから起きる:

- **入れ替えの元にする並びは`items`(props)ではなく`working`(ref)から取る。** `onReorder`で渡した並びが`items`として返ってくるのは描き直しのあとなので、速く動かすと**まだ1つ前の`items`から`move`することになり、直前の入れ替えが無かったことになる**。1行ずつ運んだつもりが1回ぶんしか動かない、という形で出る(実測: 9行目を先頭まで運んでも、隣同士が1回入れ替わっただけの並びが保存された)
- **移動と終了は`window`で受ける**(ハンドルには`onPointerDown`だけ載せる)。並べ替えると行のDOMが動くので、そこでポインタの捕捉(`setPointerCapture`)が外れることがある。外れると以降のイベントは指の下の別の要素へ行くため、**指を離しても`onCommit`が呼ばれず、画面は入れ替わったまま保存だけされない**(エラーも出ない)

**速さを変えて確かめること。** ゆっくり動かすと描き直しが毎回間に合うので、どちらも表に出ない —— 手元でゆっくり試して動いたことは、動く根拠にならない。
