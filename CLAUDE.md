...
- `UNCLUSTERED_LAYER_ID`のクリックで`e.features`が複数あるとき(=タップ位置に複数のピンが重なっている)は、先頭を開かず**「この地点のスポット」一覧**(`stackSpotIds` state)を出してどれを開くか選ばせる。選ぶと通常どおり`handleSpotSelect`に流れるため、作成モード中の追加確認にもそのまま繋がる
