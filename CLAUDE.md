...
**プロンプトはこちらが正**(`collect_prompt`)。何を集めさせるかは種別の設定の一部で、
管理画面の「情報を集めさせる」で書いて依頼する。

| 口 | やること |
|---|---|
| `GET /api/spots/collect?type=` | 状態(収集の有無・有効か・前回の結果)といまのプロンプト |
| `POST /api/spots/collect?type=` | 依頼する(まだ無ければ`POST /v1/collect`、あれば`PATCH`) |
| `POST /api/spots/collect/run?type=` | 予定を待たずに1回集めさせる(`POST /v1/collect/{name}/run`) |
...
**書き換えてから起こす**だけで足りる。書き換えるのは1回ぶんで、AIが`next_cursor`を
...
`DiscoverySource`に`collect`を足し、地図の印もバッジも専用の色(teal)にする。
**中心が無いので探す範囲の円は出さず、距離も出さない**。代わりに、取り出したあと
座標の引けた候補全体が入るよう地図を寄せる。
