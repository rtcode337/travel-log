...
**よく使う期間はカレンダーを開かずに選べる。** 訪問日セクションとカレンダーに**「今日」「表示しない」**を並べ、見出し行の「これだけを表示」の隣に**「過去1年だけを表示」**を置いてある(`MapView`の`visitDateOptions`)。**1年前の日付は`Date`に年だけ引かせる**ので、2月29日は3月1日に送られる(閏年でない年に2月29日は無い)——絞り込みの範囲としては1日の差なので、月末を特別扱いはしない。
