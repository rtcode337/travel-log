...
**スポット詳細では説明文の出どころを本文から切り離して出す。** 説明文の末尾に付いている`(出典: …)`・`(掲載元: …)`を括弧ごと外し、本文の下に小さく1行ずつ置く(`SpotDetailModal`の`descriptionBody`/`descriptionSource`)。括弧書きのまま本文に続けると説明の一部のように読め、行も伸びる。**URLは`%28`/`%29`で書く決まり**(travel-log-data側)なので、閉じ括弧までを出どころとして切ってよい。

**そのスポット自身のWikipedia記事へのリンクは、説明文に記事そのもののURLが無いときだけ出す。** `掲載元`は「◯◯の観光地」のような一覧ページを指していてスポット自身の記事へ辿れないが、`出典`は記事そのもののURLなので二重になる。記事名は`wikipedia_title_source`に従う。**かつてスポット詳細にあったWikipediaのアイコンボタン(押すと記事の概要を`SpotInfoModal`で出す)は廃した** —— 同じ記事に行く導線が2つあるうえ、アイコンの並ぶ行が長くなっていた(`SpotInfoModal`自体は地図の「訪問予定に追加」の確認から今も使う)。
