...
| `/[type]/account` | 自分のロール表示、ログアウト、訪問記録のエクスポート(ZIP)、他のスポット種別への切り替え |
...
- **訪問記録のエクスポート**: `/[type]/account`から、自分の全訪問記録をZIPで一括ダウンロード
  できる(`visits.csv`=訪問のメモ+スポット情報、`photos/`=添付写真。全スポット種別分をまとめて出力)
