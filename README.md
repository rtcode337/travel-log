...
- **未訪問記録**(非公開メモ)。訪問したけれど休みや時間の都合でちゃんと見られなかったときの記録や、事前の下調べのメモを、訪問記録とは別に残せる(訪問済みにはならない)
...
| パス | 内容 |
|---|---|
| `/[type]/map` | 地図(ホーム)。シリーズ・訪問状態・カテゴリでフィルタ。ピンタップ→スポット詳細モーダルへ。別の種別のダウンロード済み公開スポットを半透明で重ねて表示することもできる(タップで読み取り専用の詳細を表示)。左下に今表示中のスポット種別名を小さく表示 |
...
- **未訪問記録(非公開)**: 休みや時間の都合でちゃんと見られなかったときの記録や、事前の
  下調べのメモ。日時(任意)+メモだけの軽い記録で、訪問済みにはならず、訪問記録を
  つけても自動では消えない。`/[type]/spots`に自分の未訪問記録の一覧が出る
