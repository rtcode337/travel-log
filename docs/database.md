...
- **写真の実体は DB に入れない**。`visits.photos` は `text[]` の相対パス
  ...
  `data/photos/` に置く(`lib/photos.ts`)。DB と写真は同じ `data/` の下にあるので、バックアップはそのコピー1つで足りる
...
erDiagram
    ...
    spot_flags {
        ...
        timestamptz forwarded_at "受け取る側へ渡した日時(空=未依頼)"
    }
    spot_routes {
        uuid id PK
        uuid spot_type_id FK
...
- **`spot_flags` は「修正・追加の依頼」**(画面の表示名)。管理者(spot_admin/admin)が
  ...
  管理画面の一覧に出て、まとめて渡すテキストにするか、削除するかで片付ける。
  渡したら `forwarded_at` に日時を入れ、空の「未依頼」と入っている「対応中」を見分ける
